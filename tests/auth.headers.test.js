...
describe('Authentication headers', () => {
  ...
  it('accepts any client auth header for Anthropic-native endpoint and sends correct upstream header', async () => {
    const upstream = await startUpstreamMock({ mode: 'json', endpoint: 'anthropic' })
    const proxyPort = 3215
    const child = await spawnProxyProcess({
      port: proxyPort,
      baseUrl: `http://127.0.0.1:${upstream.port}`,
      env: {
    ...
    try {
      ...
      // Verify the proxy sends correct upstream header regardless of client header
      const headers = upstream.received.headers
      expect(headers['x-api-key']).toBe('anth-key')
      expect(headers['authorization']).toBeUndefined()
      expect(headers['anthropic-version']).toBe('2023-06-01')
  ...
  it('accepts any client auth header for OpenAI-compatible endpoint and sends correct upstream header', async () => {
    const upstream = await startUpstreamMock({ mode: 'json', endpoint: 'openai' })
    const proxyPort = 3216
    const child = await spawnProxyProcess({
      port: proxyPort,
      baseUrl: `http://127.0.0.1:${upstream.port}`,
      env: {
